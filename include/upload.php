...
class upload{
	...
	function init($attach, $type = 'common', $extid = 0, $forcename = '') {
    global $_S;
		if(!is_array($attach) || empty($attach) || !$this->is_upload_file($attach['tmp_name']) || trim($attach['name']) == '' || $attach['size'] == 0) {
			...
			$attach['attachment'] = $this->type.'/'.$attach['attachdir'].$this->get_target_filename($this->extid, $this->forcename).'.'.$attach['extension'];
			$attach['target'] = $_S['atcdir'].'/'.$attach['attachment'];
			$this->attach = & $attach;
			$this->errorcode = 0;
			return true;
		} 
	...
	function errormessage() {
		...
		return $errormessage[$this->errorcode];
	}
	...
	function get_image_info($target, $allowswf = false) {
		...
		$isimage = upload::is_image_ext($ext);
	...
	function get_target_filename($extid = 0, $forcename = '') {
		$filename = $extid.($forcename != '' ? "_$forcename" : '');
		return $filename;
	}

	function get_target_extension($ext) {
		static $safeext  = array('attach', 'jpg', 'jpeg', 'gif', 'png', 'bmp', 'txt', 'zip', 'rar', 'mp3', 'mp4', 'mov');
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		$check_exists && upload::check_dir_exists($type, $subdir1, $subdir2);
    ...
    global $_S;
		...
		$basedir = !$_S['atc'] ? (ROOT.'./data/atc') : $_S['atc'];
		...
		if(!$res) {
			$res = $typedir && upload::make_dir($typedir);
			$res && $subdir1 && ($res = upload::make_dir($subdir1));
			$res && $subdir1 && $subdir2 && ($res = upload::make_dir($subdir2));
		}
	...
	function save_to_local($source, $target) {
		if(!upload::is_upload_file($source)) {
			$succeed = false;
