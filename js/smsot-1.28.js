// JavaScript Document
var backfun='',intervalobj,count = 60,curcount,evalscripts = [],hackcsss = [],POPMENU = new Object,autobtn={},auto=[],loadstatus = true,UA = navigator.appVersion,scrollTop_end = 0,scroll_times;
...
$(document).on('click', '.get', function() {
  ...
  //switch
	if(type=='switch'){
		...
		var translate=Math.round((window.innerWidth/tabs.length-14)/2+(window.innerWidth/tabs.length)*current,2);
...
$(document).on('click', '.playvideo', function() {
	...
	obj.parent().append('<video '+iosadd+'controls src="'+src+'" id='+videoid+'></video>');
	obj.remove();
	$('#'+videoid)[0].play();
	return false;	
});
	...
	returnload : function(s,type) {
		...
		if(type!='reload'){
			if(type=='tab'){
				...
				setTimeout(function(){$('#loadpage').css({'display':'none','-webkit-transform':'translateX(100%)','-webkit-transition':'-webkit-transform 0s 0s'});},1);
			}			
		}
...
　　return str;
  ...
  /*deleteimg*/
	...
	translate_int : function() {
		...
		var num=tabs.length;
		var translate=Math.round(((window.innerWidth/2)-14)/num,2);
		if(now>1){
			var translate=Math.round(translate+(now-1)*(window.innerWidth/num),2);
		}
		$('.currentbody .swipernv-on').css({'-webkit-transform':'translateX('+translate+'px)','-webkit-transition':'-webkit-transform 0.3s ease-out 0s'});
	},
	/*closepage*/
	...
	postmessage : function(url,title,img){
		...
		var img=img||'';
		if(SMS.empty(img) || img==SITEURL+'ui/ico.png'){
			var img=SITEURL+'ui/shar.png';
		}
