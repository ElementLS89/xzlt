var UPLOAD = {
	buildfileupload: function(s) {
		try {
			...
			if(1 || (s.maxfilesize && s.file.size > s.maxfilesize * 1024)) {
				canvaszoom = true;
			}
			...
			var picupload = function(picdata) {
				...
				builder = UPLOAD.getbuilder(s, file.name, picdata, boundary);
			...
			img.onload = function() {
				...
				if(typeof atob == 'function') {
					...
					imgfilebinary = UPLOAD.base64decode(imgfilebinary);
				}
				...
				var newdataurl = canvas.toDataURL(s.file.type,1.0).replace(/data:.+;base64,/, '');

				if(typeof atob == 'function') {
					picupload(atob(newdataurl));
				} else {
					picupload(UPLOAD.base64decode(newdataurl));
				}
			};
			...
			if(canvaszoom) {
				...
				reader.readAsBinaryString(s.file);
			}
		} catch(err) {
			return s.error();
		}
